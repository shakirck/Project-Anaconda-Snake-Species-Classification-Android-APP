        ...
        if (isOnline() == true) {
            ...
            StringRequest request = new StringRequest(Request.Method.POST, URL, new Response.Listener<String>() {
                ...
                public void onResponse(String s) {
                    ...
                    ArrayList<SnakeSpecies>  snakeSpecies = new ArrayList<>();
                    JsonObject jsonObject = new JsonParser().parse(s).getAsJsonObject();
                    for (Map.Entry<String, JsonElement> lesion : jsonObject.getAsJsonObject("predictions").entrySet()) {
                        SnakeSpecies snakeclass = new SnakeSpecies();
                         snakeclass. setSnakenType(lesion.getKey());
                         snakeclass.setProbability(lesion.getValue().toString().replaceAll("^\"|\"$", ""));
                         snakeSpecies.add( snakeclass);
                    }
                    //sort predictions based on their probabilities value
                    Collections.sort( snakeSpecies, Collections.reverseOrder());
            ...
            request.setRetryPolicy(new DefaultRetryPolicy(
                    500000,DefaultRetryPolicy.DEFAULT_MAX_RETRIES,DefaultRetryPolicy.DEFAULT_BACKOFF_MULT
            ));
            rQueue.add(request);
        } else {
